...
class Search {
    ...
    view(vnode) {
        ...
        return section.section.fade(
            div.container(
                !!this.selected ? m(QuickView, {
                    isActive: !!this.selected,
                    header: [
                        h2.title(this.selected.name),
                        span.delete({ onclick: () => this.selected = null })
                    ],
                    footer: [
                        a({ href: 'https://de.wikipedia.org' }, span.mdi.mdiWikipedia(), ' Unsere Quelle')
                    ]
                }, []) : null,
